...
function transferTokenValue(to, coin, fiat) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        var seAmt = (parseFloat(fiat) / (allTokens[coin].rate * baseX) * Math.pow(10, allTokens[coin].decimals));
        ...
        if (coin == 'eth') {
            ...
            trnDat.data = allTokens[coin].contract.transfer.getData(to, parseInt(seAmt), {
                from: myWalt
            });
    ...
    return new Promise(function (resolve, reject) {
        ...
        setTimeout(function () {
            try {
                ...
                console.log("Drive failed to load coz " + err)
            } finally {
                var initialRequest = gapi.client.drive.files.list({
                    'q': '\'appfolder\' in parents'
                });
