...
function TokenListLayout() {
  ...
  const filteredTokens = useMemo(() => {
    ...
    return tokens.filter(token => {
      ...
      return showZeroBalances || balance > 0;
    });
  ...
  const topTokens = useMemo(() => {
    ...
    return filteredTokens.slice(0, 10);
  }, [filteredTokens]);
      ...
      <div className="flex justify-between w-full border-b border-element-10 py-4">
        <div className="flex items-baseline">
          ...
          <span className="text-0.875 ml-2 flex text-medium">
            ({formatCurrencyWithSymbol(totalTokenValue, ChainID.HederaTestnet)})
          </span>
        </div>
        ...
        {tokens.length > 0 && (
          <button
            ...
            className="text-low text-0.875 hover:text-high transition-colors"
          >
